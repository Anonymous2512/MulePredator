...
const C = {
  ...
  panel: "#FFFFFF",
  panelHi: "#F5F7F8",
  line: "rgba(139,127,224,0.18)",
  lineHi: "rgba(139,127,224,0.32)",
  text: "#161B21",
  textDim: "#5B6673",
  textFaint: "#8993A1",
  monitor: "#B5790F",
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: "auto" }}>
      ...
      {spokeIds.map((id) => {
        const p = pos[id];
        if (!p) return null;
        return <circle key={id} cx={p.x} cy={p.y} r={8} fill="rgba(122,107,217,0.55)" stroke={C.bg} strokeWidth={2} />;
      })}
    </svg>
  );
}
                ...
                <div key={a.txn_id} onClick={() => setSelected(a)} style={{ padding: "11px 14px", borderBottom: `1px solid ${C.line}`, borderLeft: `3px solid ${meta.color}`, background: sel ? C.panelHi : "transparent", cursor: "pointer" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
                    <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: 0.8, color: meta.color, fontFamily: "'JetBrains Mono',monospace" }}>
                      ...
                      {a.trigger_type === "Critical Single-Engine Override" && <span style={{ color: C.high, marginLeft: 6 }}>· OVERRIDE</span>}
                      {a.trigger_type === "Converged" && <span style={{ color: C.clear, marginLeft: 6 }}>· CONVERGED</span>}
                    </span>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {selected && (
            <div style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 8, padding: 18 }}>
              ...
              {selected.trigger_type && selected.trigger_type !== "None" && (
                <div style={{ fontSize: 10, letterSpacing: 1, color: C.textDim, fontFamily: "'JetBrains Mono',monospace", marginBottom: 8 }}>
                  TRIGGER: <span style={{ color: selected.trigger_type === "Converged" ? C.clear : C.high }}>{selected.trigger_type === "Converged" ? "CONVERGED" : "CRITICAL OVERRIDE"}</span>
                </div>
              )}
  ...
  const mapped = apiTxns
    ...
    .map((a) =>
      mkTxn({
        ...
        trigger_type: a.trigger_type || "None",
